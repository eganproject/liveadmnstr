...
<nav class="flex mb-4" aria-label="Breadcrumb">
    <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
        ...
        <li aria-current="page">
            <div class="flex items-center">
                <i data-lucide="chevron-right" class="w-4 h-4 text-gray-400"></i>
                <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2">Jabatan</span>
            </div>
        </li>
    </ol>
</nav>
...
@section('content')
    <div class="bg-white p-6 rounded-lg shadow-sm mb-6">
        <h3 id="form-title" class="text-lg font-semibold text-gray-800 mb-4">Tambah Jabatan</h3>
        @if ($errors->any())
        ...
        <form id="jabatan-form" action="{{ route('admin.masterdata.jabatan.store') }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="name" class="block text-xs font-medium text-gray-600 mb-1">Nama Jabatan</label>
                    <input type="text" id="name" name="name" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-black text-sm" placeholder="Masukkan nama jabatan" required value="{{ old('name') }}">
                </div>
                <div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-sm">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">Daftar Jabatan</h3>
        <div class="overflow-x-auto">
            <table id="jabatanTable" class="display" style="width:100%">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Nama Jabatan</th>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="flex items-center space-x-2 justify-center">
                                <button class="p-1 text-blue-600 hover:text-blue-800 edit-btn" data-id="{{ $jabatan->id }}" data-name="{{ $jabatan->name }}" data-description="{{ $jabatan->description }}">
                                    <i data-lucide="file-pen-line" class="w-4 h-4"></i>
                                </button>
                                <form class="delete-form" action="{{ route('admin.masterdata.jabatan.destroy', $jabatan->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
...
<script>
    $(document).ready(function() {
        ...
        $('.edit-btn').on('click', function() {
            ...
            formTitle.text('Edit Jabatan');
            nameInput.val(name);
            descriptionInput.val(description);
            
            form.attr('action', `/admin/masterdata/jabatan/${id}`);
